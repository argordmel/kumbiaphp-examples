...
class DwPaginate {
     ...
     * cursor: numero de pagina actual <br>
     ...
     * total_items: Total de registros <br>
    ...
    public static function paginate($model) {
        ...
        $page_number = (!empty($params['page']) && Filter::get($params['page'], 'page') > 0) ? Filter::get($params['page'], 'page') : 1; //Numero de la página
        $per_page = (!empty($params['per_page'])  && Filter::get($params['per_page'], 'page') > 0) ? Filter::get($params['per_page'], 'page') : PER_PAGE; //Datos por página
        ...
        if (is_array($model)) {
            ...
            if ($page_number > 1 && $start >= $n) {
                Flash::error('La página solicitada no se encuentra en el paginador.');
            }
            $total_items = $n;
        ...
        if( ($page->total_pages < $page_number) && ($total_items > 0)) {
            $page->prev = false;
            Flash::error('La página solicitada no se encuentra en el paginador.');
        }
        $page->cursor = ($total_items >= $counter) ? $counter : 1;
