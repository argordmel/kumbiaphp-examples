...
class DwUtils {
    ...
    /**
     * Método para devolver un numero en moneda
     */
    public static function getN2M($number, $decimals=NUMBER_DECIMALS, $sep_dec=NUMBER_SEP_DEC, $sep_mil=NUMBER_SEP_MIL) {         
        $decimals = Filter::get($decimals, 'int');
        if(empty($number)) {            
            $number = empty($decimals) ? 0 : str_pad('0.', $decimals, 0, STR_PAD_RIGHT);
        } else if($number == '-') {
            $number = 0;
        }
        return number_format($number, $decimals , $sep_dec, $sep_mil);
    }
    
    /**
     * Método para devolver una moneda en letras
     */
    public static function getM2L($valor, $moneda='PESOS', $centavos=0) {
        $a = $valor;
        $p = $moneda;
        $c = $centavos;
        $val = "";
    ...
    /**
     * 
     * Método para copiar un directorio a otro destino
     * @param string $source
     * @param stiing $target
     */
    public static function copyAllFolder( $source, $target, $chmod=0777 ) { 
        if ( is_dir( $source ) ) { 
            @mkdir( $target );
            @chmod($target, $chmod);
            $d = dir( $source ); 
            while ( FALSE !== ( $entry = $d->read() ) ) { 
                ...
                $Entry = $source . '/' . $entry; 
                if ( is_dir( $Entry ) ) { 
                    self::copyAllFolder( $Entry, $target . '/' . $entry ); 
                    continue;                     
                } 
                copy( $Entry, $target . '/' . $entry );
                @chmod( $target . '/' . $entry, $chmod );
            } 
            $d->close(); 
